...
db.collection("users")
  ...
  .where("ingredientName", "==", "cotlet de porc")
  .limit(10)
  .get()
  .then(snap => {
    if (snap.empty) {
      console.log("Nu s-au gasit intrari cu cotlet de porc");
      process.exit(0);
    }
    console.log("Gasit", snap.size, "intrari cu cotlet de porc:");
    snap.forEach(doc => {
      const d = doc.data();
      console.log("\nDoc ID:", doc.id);
      ...
      console.log("  carbs:", d.carbohydrates, "g");
      console.log("  fat:", d.fat, "g");
      if (d.amount > 0) {
        const energyPer100g = (d.energy / d.amount * 100).toFixed(1);
        console.log("  => Calculat energy/100g:", energyPer100g, "kcal/100g");
        console.log("  => Ar trebui sa fie: 155 kcal/100g");
      }
    });
    process.exit(0);
  })
